...
| 平台 | 文件 | 说明 |
| --- | --- | --- |
| Windows x64 | `CodexTokenOverlay-win-x64.zip` | 大多数 Intel、AMD Windows 电脑。 |
| Windows Arm64 | `CodexTokenOverlay-win-arm64.zip` | Windows on Arm 设备。 |
...
每个 ZIP 旁都提供 `.sha256` 校验文件。发行包已包含运行所需组件：Windows 用户无需安装 .NET 或 PowerShell，macOS 用户无需安装 Xcode、Swift 或 Homebrew。

## Windows 使用方法

1. 下载与 Windows 架构对应的 ZIP。
2. 解压到任意位置。
...
> [!NOTE]
> macOS 版本当前标记为 **Beta**：日志解析器、IPC 状态机、应用包、签名和两种 CPU 架构均由 CI 验证，但“真实 Codex 中切换未运行任务”的最终验收仍需要在实体 Apple Silicon Mac 上完成。
